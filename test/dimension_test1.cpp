...
void dimension_test1()
{
   ...
   // n.b must be sorted
   typedef dimension<exp_time<1> > dim1;
   typedef merge_dim<dim1,pqs::times,dim1 >::type result1;
   typedef dimension<exp_time<2> > expected1;
  ...
  // int x = result1{};
   ...
   typedef dimension<exp_length<-2>, exp_time<2> >  dim2;
   typedef merge_dim<dim1,pqs::times,dim2 >::type result2;
   typedef dimension<exp_length<-2>, exp_time<3> > expected2;
 //  static_assert(std::is_same<result2,expected2>::value,"");
   ...
   typedef dimension<exp_length<2>, exp_time<-1> > expected2a;
   static_assert(std::is_same<result2a,expected2a>::value,"");

   typedef dimension<exp_length<-1>, exp_time<1>, exp_mass<2> >  dim3;
   typedef merge_dim<dim2,pqs::times,dim3 >::type result3;
   typedef dimension<exp_length<-3>, exp_mass<2>, exp_time<3>  > expected3;
   static_assert(std::is_same<result3,expected3>::value,"");

   typedef merge_dim<dim2,pqs::divides,dim3 >::type result3a;
   typedef dimension<exp_length<-1>,exp_mass<-2>,exp_time<1> > expected3a;
   static_assert(std::is_same<result3a,expected3a>::value,"");

   typedef dimension<exp_length<1>, exp_time<1>, exp_mass<2> >  dim4;
   typedef merge_dim<dim3,pqs::times,dim4 >::type result4;
   typedef dimension<exp_mass<4>,exp_time<2> > expected4;
   ...
   typedef dimension<exp_length<2>,exp_mass<4>,exp_time<2> >::type expected5a;
